...
// only domain addition is `user.role` (viewer | coordinator) carried on the session.
// Synthetic accounts only — never any real PII (S13).

import { sqliteTable, text, integer } from "drizzle-orm/sqlite-core"

// The two-tier role hierarchy. Kept as a literal union so guards compare known values.
export const ROLES = ["viewer", "coordinator"] as const
...
export const user = sqliteTable("user", {
  ...
  image: text("image"),
  // Domain role; seed-only this release. Defaults to the least-privileged tier.
